...
扫描 `{notes_root}/`（排除 `_archive/`）中的敏感信息。使用 `grep_search` 或直接读取文件。

### 检测模式

| 类别 | 模式 / 关键词 |
|------|-------------|
| 密码 | `password`、`passwd`、`pwd`、`密码`、`secret` 等靠近 `=` 或 `:` |
| 身份证 | 18 位中国身份证模式：`/\d{6}(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\d{3}[\dXx]/` |
| 社保号 (SSN) | 美国 SSN：`/\d{3}-\d{2}-\d{4}/` |
| 护照号 | `passport`、`护照`，模式：`/E\d{7,8}/`、`/G\d{7,8}/`（中国）、`/\d{9}/`（美国） |
| 驾照号 | `driver`、`驾照`、`DL-`、`license` 等靠近字母数字组合 |
| 银行卡号 | 16–19 位数字串，`card`、`银行卡`、`credit` 等 |
| API 密钥 / 令牌 | `api_key`、`token`、`secret`、`sk-`、`ghp_`、`gho_`、`Bearer`、`Authorization` |
| 手机号码 | 中国手机号 `1[3-9]\d{9}`，国际号码 `\+\d{7,15}` |
| 邮箱 | `[...]@[...]` — 在异常上下文中出现时标记 |

### 执行流程

1. 告知："正在扫描笔记中的敏感信息……"
2. 在 `{notes_root}/` 范围内搜索（跳过 `_archive/` 和 `.templates/`）
3. 对每个匹配项：
   - 报告文件路径和行号
   - 显示匹配类别（**绝不输出实际的敏感值**）
